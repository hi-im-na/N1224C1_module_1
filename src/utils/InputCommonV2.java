    ...
    public <T extends Number> T checkNumberInput(Class<T> clazz, T min, T max) {
        while (true) {
            try {
                ...
                switch (clazz.getSimpleName()) {
                    case "Integer" -> {
                        result = clazz.cast(Integer.parseInt(checkInputString()));
                        if (result.intValue() < min.intValue() || result.intValue() > max.intValue()) {
                            throw new NumberFormatException();
                        }
                    }
                    case "Double" -> {
                        result = clazz.cast(Double.parseDouble(checkInputString()));
                        if (result.doubleValue() < min.doubleValue() || result.doubleValue() > max.doubleValue()) {
                            throw new NumberFormatException();
                        }
                    }
                    default -> throw new IllegalArgumentException("Invalid class type or not developed");
